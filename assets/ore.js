/**
 * Saraaf Trade Toolkit - Ore & Concentrate Valuation Engine
 */

function runOreValuation() {
  const dryWeightMT = parseFloat(document.getElementById('ore-dry-weight').value);
  const gradePct = parseFloat(document.getElementById('ore-grade-pct').value);
  const benchmarkPriceUSD = parseFloat(document.getElementById('ore-benchmark-price').value);
  ...
  if (isNaN(dryWeightMT) || isNaN(gradePct) || isNaN(benchmarkPriceUSD) || dryWeightMT <= 0 || gradePct <= 0 || benchmarkPriceUSD <= 0) {
    document.getElementById('ore-results-card')?.classList.add('hidden');
    return;
  }
  ...
  document.getElementById('output-ore-net-value').textContent = 
    '$' + netSettlementUSD.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
  
  document.getElementById('output-ore-per-mt').textContent = 
    '$' + netValuePerMT.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 }) + ' / DMT';
  ...
  document.getElementById('ore-results-card')?.classList.remove('hidden');
}

// Native Share API (Triggers WhatsApp, AirDrop, Messages on Mobile)
function shareOreResults() {
  ...
  const shareText = `Saraaf Ore Valuation | Net Lot Value: ${totalVal} (${perMtVal})`;
